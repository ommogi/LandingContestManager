...
const HighlightCard = ({ title, description, icon, variant = "default" }: HighlightCardProps) => {
  const isAccent = variant === "accent";

  return (
    <div className="group cursor-pointer transform transition-all duration-500 hover:scale-105 hover:-rotate-1 h-full">
      <Card className="text-white rounded-2xl border border-white/10 bg-gradient-to-br from-[#010101] via-[#090909] to-[#010101] shadow-2xl relative backdrop-blur-xl overflow-hidden hover:border-white/25 hover:shadow-3xl hover:shadow-white/5 w-full h-full min-w-0">
        <div className="absolute inset-0 z-0 overflow-hidden">
          ...
          <div className={`absolute -bottom-20 -left-20 w-48 h-48 rounded-full blur-3xl opacity-30 group-hover:opacity-50 transform group-hover:scale-110 transition-all duration-700 animate-bounce ${isAccent ? "bg-gradient-to-tr from-[var(--gold)]/30 to-transparent" : "bg-gradient-to-tr from-white/10 to-transparent"}`}></div>
        ...
        <div className="p-8 relative z-10 flex flex-col items-center text-center">
          {isAccent && (
            <span className="mb-4 px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full bg-[var(--gold)] text-[#07090d]">★</span>
          )}
          ...
          <h3 className={`mb-4 text-3xl font-bold bg-clip-text text-transparent animate-pulse transform group-hover:scale-105 transition-transform duration-300 ${isAccent ? "bg-gradient-to-r from-[var(--gold)] via-[var(--gold)] to-[var(--text)]" : "bg-gradient-to-r from-white via-gray-100 to-white"}`}>
            {title}
          </h3>
            ...
            {description.map((line, idx) => (
              <p
                ...
                className={`leading-relaxed transition-colors duration-300 group-hover:text-gray-200 ${
                  idx === 0 ? "text-lg font-semibold text-[var(--accent)]" : "text-gray-300 text-sm"
                }`}
              >
                {line}
              </p>
